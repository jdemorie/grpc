...
## gRPC-web: générer le client TypeScript

Le fichier proto est dans `proto/version.proto`.

Le code TypeScript (messages + client) est généré dans `src/gen/`.

### Prérequis

Installer les dépendances (à faire une seule fois après un clone) :
...
Commande recommandée (script NPM) :

```bash
npm run gen
```

Équivalent direct (si besoin) :
